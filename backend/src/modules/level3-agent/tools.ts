...
import type { InterruptionMode, ToolOption } from "./settings.js";
...
function interruptionModeForTools(
  mode: InterruptionMode,
): ElevenLabs.ToolInterruptionMode {
  if (mode === "protect_tools") return "disable_during_tool_and_turn";
  return "allow";
}

const ALL_TOOLS: Record<ToolOption, Omit<ClientTool, "interruptionMode">> = {
...
export function buildLevel3ClientTools(
  enabledTools: ToolOption[],
  interruptionMode: InterruptionMode,
): ClientTool[] {
  const toolInterruption = interruptionModeForTools(interruptionMode);
  return enabledTools.map((name) => ({
    ...ALL_TOOLS[name],
    interruptionMode: toolInterruption,
  }));
}
