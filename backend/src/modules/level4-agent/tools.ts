...
const CLINICAL_TOOLS: Record<string, Omit<ClientTool, "type">> = {
  ...
  flag_watch_event: {
    ...
    parameters: {
      type: "object",
      properties: {
        level: stringProp("Severity of the watch event", [
          "info",
          "warning",
          "critical",
        ]),
...
export function buildLevel4MemoryTool(): ClientTool {
  return {
    type: "client",
    name: "query_memory_bank",
    description:
      "Look up facts from the caller-provided memory bank for this session. Use before inventing personal details such as medications, history, preferences, caregivers, or preferred pharmacies.",
    expectsResponse: true,
    ...TOOL_SOUND,
    parameters: {
      type: "object",
      properties: {
        query: stringProp(
          "Optional focus query (e.g. medications, pharmacy, allergies). Leave broad to return the full bank.",
        ),
      },
      required: [],
    },
...
/** Official Exa + ElevenLabs webhook tool (instant + highlights for voice latency). */
export function buildLevel4ExaWebSearchTool(exaApiKey: string): WebhookTool {
  return {
    type: "webhook",
    name: "web_search",
    description:
      "Search the web using Exa. Use this when the user asks anything that needs current or factual information.",
...
export function buildLevel4ClientTools(): ClientTool[] {
  ...
  return [buildLevel4MemoryTool(), ...clinical];
}
